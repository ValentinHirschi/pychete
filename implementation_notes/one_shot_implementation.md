...
- Current first-milestone clarification after the latest diagnostics: no full
  Matchete one-loop matching integration test is reproduced yet. Re-running a
  broader selected Wilson-line plan over `hScalar`, `hScalar-hScalar`, and
  `hScalar-lScalar` with `max_total_order=4` produced seven finite entry keys
  and 25 Wilson-line terms. Entrywise projection still finds only the
  `hScalar-lScalar` order-four entry, now named
  `hScalar-lScalar#wilson34_o4_0` in the broader plan, with the same
  normalized coefficient
  `-1/2*hbar*log(M) + 1/4*hbar*log(M^2) + 7/24*hbar` times
  `A^2*gL^2/M^4`; the logs cancel under `log(M^2)=2 log(M)`. The aggregate
  projection of the larger combined source can still return zero, which is a
  separate projection guard/scaling issue and not evidence that the entrywise
  physics contribution vanished.
- Matchete re-read for this frontier: `PropBosonExpand` agrees with pychete's
  bosonic propagator expansion formula, and native vakint tensor reduction
  emits dimension-dependent rank-two/rank-four factors such as
  `1/(-4+2*epsilon)` and `1/(24-20*epsilon+4*epsilon^2)`, so a fixed
  four-dimensional tensor average is not the obvious mismatch. The Singlet
  `hScalar-lScalar` Wilson-line path closes on the heavy singlet `phi`; the
  relevant `cHW` field strengths are generated by open derivatives acting on
  charged Higgs insertions, not by expanding a charged closing WilsonTerm.
  Matchete's `ContractMetric` only substitutes metric-contracted derivative
  indices, while `CommuteCDs` and basis identities enter later through
  normal-form/operator simplification. This means the next generic physics
  implementation should move the Wilson-line result through a Matchete-style
  loop-symmetric commutator and basis-normal-form reduction, using Symbolica
  patterns plus idenso-backed algebra, instead of correcting the `7/24`
  coefficient with a target-specific patch.
- Latest implementation slice: added a bounded Matchete-adjacent commutator
  mode to the existing Symbolica-pattern emitter. The stable default remains
  `Theory.emit_covariant_derivative_commutators(..., mode="inversions")`,
  which commutes only out-of-canonical-order adjacent derivative slots and can
  be used for repeated canonicalization. The new
  `mode="all_distinct"` emits the first distinct adjacent derivative-pair
  identity in the local Matchete `CommuteCDs` sense and is deliberately limited
  to one pass to avoid swapping the same pair back in a repeat loop. The
  Wilson-line route exposes this through
  `OneLoopMatchOptions.wilson_line_covariant_derivative_commutator_mode`, and
  result metadata records both top-level
  `wilson_line_commutator_emit_mode` and interaction-level
  `interaction_wilson_line_commutator_emit_mode`.
- Verification for that slice: `py_compile` passed for `theory.py`,
  `matching.py`, and `matching_options.py`; focused definition tests passed
  with `10 passed, 44 deselected`; the full definition unit file passed with
  `54 passed`; the selected Wilson-line integration smoke passed with
  `1 passed, 103 deselected`; and source mypy passed for the three touched
