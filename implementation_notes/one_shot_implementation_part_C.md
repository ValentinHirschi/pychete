...
## Current Slice: Opt-In Abelian Covariant-Derivative Expansion

- Added `Theory.expand_abelian_covariant_derivatives(...)` as an explicit
  Symbolica-replacement pass for Matchete-style expressions where Abelian
  gauge interactions are implicit in first-derivative `Field` slots. The pass
  uses registered field/group symbol data and `_abelian_gauge_connection(...)`;
  global U(1) charges are ignored and non-Abelian covariant derivatives remain
  untouched for the idenso/spenso-backed group-algebra path.
- Added a central `CovariantDerivativeProtectedBar` temporary head in
  `SymbolStore`. Barred first-derivative atoms are collected with Symbolica
  matching, replaced by field-free temporary keys, then restored after the
  unbarred field derivative replacement pass. This avoids rewriting the field
  inside `Bar(Field(...))` a second time.
- Added `OneLoopMatchOptions.expand_abelian_covariant_derivatives`, defaulting
  to `False`, and threaded it through `match_one_loop(...)`. The public
  matching path expands the Lagrangian before `one_loop_setup(...)` only when
  requested and records `abelian_covariant_derivatives_expanded` metadata.
- Threaded the same option through `ValidationFixture.one_loop_preview(...)`
  and `one_loop_preview_gap_report(...)` so committed Matchete-independent
  validation fixtures can explicitly probe this path.
- Added regressions covering:
  - exact charged-scalar Abelian derivative expansion with the established
    `free_lag(...)` sign convention and with global U(1) charges ignored;
  - public method docstring coverage for
    `Theory.expand_abelian_covariant_derivatives`;
  - fast public `Theory.match(..., loop_order=1)` option routing using a fake
    setup that captures the preprocessed Lagrangian.
- A targeted Singlet Scalar Extension validation probe with the new option
  enabled at `max_trace_order=3` did not change the accepted projected
  matching-condition count (`42 accepted, 30 different`). This confirms the
  remaining Singlet gauge-dependent Wilson gaps require the broader
  non-Abelian/full covariant-derivative and group-algebra implementation, not
  only the scalarized Abelian first-derivative expansion.
- Targeted validation for this slice:
  - exact focused gate:
    `pytest tests/unit/definitions/test_theory_definitions.py::test_expand_abelian_covariant_derivatives_uses_symbolica_replacements tests/integration/matching/test_fluctuation_operator.py::test_one_loop_match_option_expands_abelian_covariant_derivatives_before_setup tests/unit/definitions/test_public_api.py -q`:
    7 passed.
  - definitions marker gate:
    `pytest -m definitions tests/unit/definitions -q`: 61 passed.
  - affected matching file:
    `pytest tests/integration/matching/test_fluctuation_operator.py -q`:
    53 passed.
  - `python -m mypy`: success, no issues in 33 source files.
