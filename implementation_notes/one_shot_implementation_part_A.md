...
- Final verification for the public vakint minimal-subtraction backend slice:
  ...
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    ...
    because GammaLoop was not requested in the current dependency manifest.
- Added a first-class pychete loop-function placeholder and evaluator for
  Matchete-style saved results:
  - added the central `s.LoopFunction` Symbolica head plus
    `LoopFunctionMassesWildcard` and `LoopFunctionPowersWildcard`, all created
    through `SymbolStore` so they receive pychete's custom print callbacks;
  - exposed `loop_function(...)`, `loop_function_to_vakint_integral(...)`, and
    `evaluate_loop_functions(...)` through `pychete.api` and the package root;
  - `loop_function(...)` encodes Matchete's finite `LF[{masses...},
    {powers..., alpha}]` convention, with masses stored as masses rather than
    mass-squared slots and the last power representing the massless
    `1/k^2` power generated by numerator reduction;
  - `evaluate_loop_functions(...)` uses Symbolica pattern replacement over the
    `LoopFunction` head, lowers each placeholder to the existing scalar
    vakint-topology expression, evaluates it through pychete's internal
    analytic one-loop backend, strips the common vakint imaginary-unit
    convention, and extracts the finite epsilon coefficient through native
    Symbolica coefficient extraction via `vakint.finite_part(...)`;
  - the supported-subset Matchete expression parser now maps saved-result
    `LF[...]` heads to `s.LoopFunction(...)`, giving committed fixtures a
    canonical pychete representation without requiring Mathematica at pytest
    time;
  - checked Symbolica's Python stub for `Expression.match`,
    `Expression.replace`, callable replacement right-hand sides,
    `Expression.coefficient_list`, and symbol custom print/data support before
    implementing the slice. No Python tree-walk symbolic evaluator was added.
- Final verification for the loop-function placeholder/evaluator slice:
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    dependencies/.venv/bin/python -m pytest
    tests/unit/backends/test_vacuum_integrals_backend.py -q'` passed: 19
    passed in 2.99s;
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    dependencies/.venv/bin/python -m pytest
    tests/unit/definitions/test_pretty_printing.py::test_loop_function_prints_cleanly_in_all_symbolica_modes
    tests/unit/definitions/test_pretty_printing.py::test_all_builtin_pychete_symbols_have_pretty_print_callbacks
    -q'` passed: 2 passed in 0.04s;
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    dependencies/.venv/bin/python -m pytest
...
- Verification for named loop-normalization staging:
  ...
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    ...
    because GammaLoop was not requested in the current dependency manifest.
- Kept normalized raw-supertrace aliases out of fixture coverage counts:
  - after normalized named traces were added, representative normalized
    internal-MS gap reports kept the same common/equal Matchete trace names but
    counted diagnostic aliases such as `hScalar[unnormalized]` as
    candidate-only supertraces;
  - added `is_unnormalized_supertrace_alias(...)` next to the shared
    Matchete-style supertrace classifier and taught `_gap_report(...)` to
    exclude only these raw-convention aliases from the validation surface;
  - `MatchingResult` objects still expose the aliases for direct inspection,
    while aggregate backend stages such as
    `interaction_power_type_internal_integral_sum`,
    `interaction_power_type_internal_integral_ms_counterterm`, and normalized
    finite/pole stage diagnostics remain visible in gap reports;
  - manually checked the normalized S1S3LQs internal-MS report now has no
    `[unnormalized]` names in `candidate_supertrace_names`, with unchanged
    common/canonical-equal trace names.
- Verification for diagnostic-alias filtering:
  - `bash -lc 'source "$HOME/.bashrc" && PYTHONPATH=src
    dependencies/.venv/bin/python -m pytest
    tests/integration/validation/test_numeric_probes.py::test_fixture_gap_report_records_supertrace_word_orders
    tests/integration/validation/test_validation_fixtures.py::test_validation_fixture_preview_can_apply_internal_normalization_without_mathematica
    -q'` passed: 2 passed in 0.92s.
