...
class OneLoopMatchOptions:
    ...
    evaluated one-loop result.  ``on_shell_eom_abelian_vector_field_redefinition``
    additionally applies the bounded scalar-current companion generated by
    Abelian vector field redefinitions, matching the first validated subset of
    Matchete's ``EOMSimplify`` vector shift. Final off/on-shell result stages
    are truncated to the requested inclusive EFT order by default before
    matching-condition projection. Matchete-style implicit Abelian and
    non-Abelian covariant derivatives can be expanded before
    fluctuation-operator extraction with
