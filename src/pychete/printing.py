...
from .serialization import _CANONICAL_PRINT_MODE, _CUSTOM_PRINT_MODE_KEY
from .symbols import SymbolDataKey, _SYMBOL_NAMESPACE
...
def print_user_symbol(expr: Expression, mode: PrintMode, **kwargs: Any) -> str | None:
...
def print_builtin(expr: Expression, mode: PrintMode, **kwargs: Any) -> str | None:
    if _is_canonical_print(kwargs):
        return None
    name = _local_name(expr)
    if name.endswith("_"):
